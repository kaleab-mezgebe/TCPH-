...
class ViewMovableDataDetail extends StatefulWidget {
  final int id;

  const ViewMovableDataDetail({super.key, required this.id});

  @override
  _ViewMovableDataDetailState createState() => _ViewMovableDataDetailState();
}

class _ViewMovableDataDetailState extends State<ViewMovableDataDetail> {
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(Uri.parse(
          'https://repositoryformymobapp-kaleab-mezgebe.onrender.com/getMovableDataa?id=${widget.id}'));
  ...
  Future<void> fetchDataa() async {
    try {
      final response = await http.get(Uri.parse(
          'https://repositoryformymobapp-kaleab-mezgebe.onrender.com/getMovableDataaa?id=${widget.id}'));
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> fetchHeritageImage() async {
    try {
      final response = await http.get(Uri.parse(
          'https://repositoryformymobapp-kaleab-mezgebe.onrender.com/getHeritageImage?id=${widget.id}'));
  ...
  Future<void> fetchImages() async {
    try {
      final response = await http.get(Uri.parse(
          'https://repositoryformymobapp-kaleab-mezgebe.onrender.com/getGalleryImages?id=${widget.id}'));
  ...
  Future<void> _downloadPDF(BuildContext context) async {
    ...
    if (Platform.isAndroid) {
      ...
      dir = Directory('/storage/emulated/0/Download/TCPH.movableHeritages');
          ...
          List<pw.Widget> content = [
            ...
            pw.Row(
              children: [
                pw.Expanded(
                  child: pw.Text(
                    'ፌርማ:___________________________________',
                    style: pw.TextStyle(font: customFont),
                  ),
                ),
              ],
    ...
    //   final file = io.File('${dir.path}/movable_data_${widget.id}.pdf');
    ...
    //     SnackBar(
    //       content:
    //           Text('PDF downloaded and saved to local storage: ${file.path}'),
    //     ),
    //   );
    // }
    final String path =
        '${dir.path}/movable_data_${widget.id}.pdf'; // Note the added .pdf extension
    File file;
